...
<div class="container">
    ...
    <ul class="nav nav-tabs" id="presupuestoTabs" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <a class="nav-link" id="not-accepted-tab" data-bs-toggle="tab" href="#not-accepted" role="tab" aria-controls="not-accepted" aria-selected="false">No Aceptados</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link" id="accepted-tab" data-bs-toggle="tab" href="#accepted" role="tab" aria-controls="accepted" aria-selected="false">Aceptados</a>
        </li>
    </ul>
    ...
    <div class="tab-content mt-3" id="presupuestoTabsContent">
                    ...
                    @foreach ($presupuestos as $presupuesto)
                        <tr>
                            <td class="align-middle">
                                <button class="btn btn-light-info" data-bs-toggle="modal" data-bs-target="#detallePresupuestoModal" data-presupuesto-id="{{ $presupuesto->id }}" data-bs-toggle="popover" title="Ver detalle de presupuesto">
                                    {{ $presupuesto->id }}
                                </button>
                            </td>
                            ...
                            <td class="align-middle">
                                ...
                                <a href="{{ route('presupuesto.edit', $presupuesto->id) }}" class="btn btn-light-primary" data-bs-toggle="popover" title="Editar presupuesto"><i class="fa-solid fa-pen-to-square"></i></a>

                                <a href="" class="btn btn-light-success create-order-btn" data-presupuesto-id="{{ $presupuesto->id }}" data-bs-toggle="popover" title="Crear orden"><i class="fa-solid fa-check"></i></a>

                                <form action="{{ route('presupuesto.destroy', $presupuesto->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="page" value="{{ $presupuestos->currentPage() }}">
                                    <button type="button" class="btn btn-light-danger delete-btn" data-presupuesto-id="{{ $presupuesto->id }}" data-bs-toggle="popover" title="Eliminar presupuesto"><i class="fa-solid fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
                ...
                <ul class="pagination">
                    ...
                    @for ($i = $startPage; $i <= $endPage; $i++)
                        <li class="page-item {{ ($presupuestos->currentPage() == $i) ? 'active' : '' }}">
                            <a class="page-link" href="{{ $presupuestos->url($i) }}&tab=all">{{ $i }}</a>
                        </li>
                    @endfor
        ...
        <div <div class="tab-pane fade {{ $tab == 'not-accepted' ? 'show active' : '' }}" id="not-accepted" role="tabpanel" aria-labelledby="not-accepted-tab">
            <table class="table table-light text-center table-hover rounded-table">
                <thead class="table-dark">
                    <tr class="align-middle">
                        <th class="icon-table">Id</th>
                ...
                <tbody>
                    ...
                    @foreach ($presupuestos->where('aceptado', 0) as $presupuesto)
                        <tr>
                            ...
                            <td class="align-middle">{{ $presupuesto->created_at }}</td>
                            <td class="align-middle">
                                <a href="" class="btn btn-light-primary" data-bs-toggle="popover" title="Editar presupuesto"><i class="fa-solid fa-pen-to-square"></i></a>
                                <a href="" class="btn btn-light-success create-order-btn" data-presupuesto-id="{{ $presupuesto->id }}" data-bs-toggle="popover" title="Crear orden"><i class="fa-solid fa-check"></i></a>
                                <form id="delete-form-{{ $presupuesto->id }}" action="{{ route('presupuesto.destroy', $presupuesto->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="page" value="{{ $presupuestos->currentPage() }}">
                                    <button type="button" class="btn btn-light-danger delete-btn" data-presupuesto-id="{{ $presupuesto->id }}"><i class="fa-solid fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="tab-pane fade {{ $tab == 'accepted' ? 'show active' : '' }}" id="accepted" role="tabpanel" aria-labelledby="accepted-tab">
            <table class="table table-light text-center table-hover rounded-table">
                <thead class="table-dark">
                    <tr>
                        <th class="icon-table">Id</th>
...
<script>
    ...
    $('#modalButtonsContainer').on('click', '.btn-send-to-client', function(e) {
        ...
        // Realizar la solicitud AJAX para enviar el correo
        $.ajax({
            url: '/phpresus/public/presupuesto/send-mail/' + presupuestoId,
            method: 'POST',
            headers: {
    ...
    $('#detallePresupuestoModal').on('show.bs.modal', function(event) {
        ...
        $.ajax({
            url: '/presus/public/presupuesto/' + presupuestoId,
            method: 'GET',
            success: function(response) {
                ...
                $('#modalButtonsContainer').html(`
                    <a href="/presus/public/presupuesto/${presupuestoId}/download" class="btn btn-light-primary"><i class="fa-solid fa-circle-down"></i> Descargar PDF</a>
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var message = "<div style='white-space: pre-line;'>Datos de la orden\n";
                        message += "Id de orden: <strong>" + orden.id + "</strong>\n";
                        message += "Fecha de creación: <strong>" + orden.created_at + "</strong>\n";
                        message += "Productos en orden:\n";
                        //DEV
                        //console.log("");
                        //DEV
                        orden.productos.forEach(function(producto) {
                            message += "Producto ID: <strong>" + producto.producto_id + "</strong>, Cantidad: <strong>" + producto.cantidad + "</strong>, Precio: <strong>" + producto.precio + "</strong>\n";
                        });
                        message += "</div>";
                        //DEV
                        //console.log(message);
                    ...
                    error: function(xhr, status, error) {
                        if (xhr.status === 400) {
                            ...
                            var messageProdElim = xhr.responseJSON.message_prod_elim;
                            var messageProdStock = xhr.responseJSON.message_prod_stock;
                            var allowForceCreate = xhr.responseJSON.allow_force_create;
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        $.ajax({
                                            ...
                                            if (xhr.responseJSON) {
                                                console.error('Detalles del error:', xhr.responseJSON);
                                            }
                                            ...
                                            if (xhr.responseJSON) {
                                                ...
                                                } else if (xhr.responseJSON.message_prod_elim) {
                                                    errorMessage = xhr.responseJSON.message_prod_elim;
                                                } else if (xhr.responseJSON.message_prod_stock) {
                                                    errorMessage = xhr.responseJSON.message_prod_stock;
                                                }
                                            }
